...
void QtAddressBookGUI::createWidgets()
{
    ...
    ecampusContactButton=new QPushButton("Ecampus Login");
    gpsContactButton=new QPushButton("GPS Tracker");
    ...
    QHBoxLayout *buttonLayout_1=new QHBoxLayout();
     buttonLayout_1->addWidget(ecampusContactButton);
     buttonLayout_1->addWidget(gpsContactButton);


    QVBoxLayout *rightSideLayout = new QVBoxLayout();
    rightSideLayout->addWidget(detailView);
    rightSideLayout->addLayout(buttonLayout);
     rightSideLayout->addLayout(buttonLayout_1);
    ...
    connect(ecampusContactButton, SIGNAL(clicked()),
           this, SLOT(authenticate()));

    connect(gpsContactButton, SIGNAL(clicked()),
            this, SLOT(editContact()));
...
void QtAddressBookGUI::deleteContact()
{
    ...
    //ErrorInfo deleteErrorStatus_1 = appController.deleteContact(idToDelete);
    if (deleteErrorStatus.code == ERR_OK)
    {
        //display error dialog
        QtErrorDialog *errDialog = new QtErrorDialog(deleteErrorStatus.msg, this);
    ...
    void QtAddressBookGUI::authenticate()
    {
        Contact newContact;
        authenticateWin *addDialog = new authenticateWin(newContact, this);
        QtErrorDialog *errDialog = new QtErrorDialog("", this);
   ...
   /* if(idToDelete==0)
    {
       QtInfoMsg *errMsg = new QtInfoMsg(" ", this);
       errMsg->exec();
       delete errMsg;
       return ;
